...
class SalObjDataset(data.Dataset):
    def __init__(self, data_root: str, img_files: list, trainsize):
        self.trainsize = trainsize
        self.images = [os.path.join(data_root, 'RGB', f) for f in img_files]
        self.gts = [os.path.join(data_root, 'GT', f.replace('.jpg','.png')) for f in img_files]
        self.depths = [os.path.join(data_root, 'depth', f.replace('.jpg','.png')) for f in img_files]
...
def get_loader(data_root: str, name_list_txt: str, batchsize: int, trainsize, shuffle=True, num_workers=6, pin_memory=True):
    with open(os.path.join(data_root, name_list_txt), 'r') as f:
        lines = f.readlines()
    img_files = [line.strip() for line in lines]

    dataset = SalObjDataset(data_root, img_files, trainsize)
...
class SalObjDataset_test(data.Dataset):
    def __init__(self, val_data_root, img_files, testsize):
    ...
    def __getitem__(self, index):
        ...
        valid_name = "NJU2K" #self.images[index].split('/')[-3]
...
def get_testloader(val_data_root : str, valid_name_list : str, batchsize, testsize, shuffle=False, num_workers=6, pin_memory=True):
    with open(os.path.join(val_data_root, valid_name_list), 'r') as f:
        lines = f.readlines()
    files = [line.strip() for line in lines]
    dataset = SalObjDataset_test(val_data_root, files, testsize)
    data_loader = data.DataLoader(dataset=dataset,
                                  batch_size=batchsize,
                                  shuffle=shuffle,
                                  num_workers=num_workers,
