    ...
    with torch.no_grad():
        ...
        for idx, batch in enumerate(test_loader):
            ...
            for b in range(B):
                ...
                mae += torch.sum(torch.abs(res_slice - gt_slice)).item() / (ori_H[b].item() * ori_W[b].item())
                cnt += 1
        ...
        name = "NJU2K"
        save_model(work_dir, epoch, model, name)
...
def main():
    ...
    with Loader("Load dataset..."):
        train_loader = get_loader(config.DATA['data_root'], 
                                config.DATA['train_name_list'], 
    ...
    # visual(device, work_dir, test_name, 1, "NJU2K")
